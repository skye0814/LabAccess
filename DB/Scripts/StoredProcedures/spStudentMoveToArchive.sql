...
ALTER PROCEDURE spStudentMoveToArchive 
	@SystemUserID int
AS
BEGIN
SET NOCOUNT ON;
	...
	BEGIN TRY
		-- Students to Archive
		SET IDENTITY_INSERT StudentsArchive ON
		...
		SELECT
			...
			ModifiedBy,
			ModifiedDate
		FROM Students WHERE SystemUserID = @SystemUserID

		SET IDENTITY_INSERT StudentsArchive OFF
		...
		SELECT 
		 ...
		 DELETE FROM Students WHERE SystemUserID = @SystemUserID
